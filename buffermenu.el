...
(defvar buffermenu-annotation nil 
  "buffermenu uses the following to display user-defined annotations for 
buffers:

buffermenu-annotation: 
    This string is included in the menu entry for all buffers which set it.
    This variable is buffer-local.

buffermenu-annotation-field-width:
    This specifies the suggested field width for such annotations. It is
    a global variable, and is used for aligning the buffer menu.")

(defvar buffermenu-annotation-field-width 0
  "buffermenu uses the following to display user-defined annotations for 
buffers:
...
(make-variable-buffer-local 'buffermenu-annotation)
...
(defun mouse-bufferkill-menu (event)
  ...
  (let ((window (and (event-over-text-area-p event)
    ...
    (when current-menubar
      (setq bmenu (or (assoc "Buffers" current-menubar)
                      (assoc "%_Buffers" current-menubar))))
    (when (null bmenu)
      (setq bmenu (or (assoc "Buffers" default-menubar)
                      (assoc "%_Buffers" default-menubar))))
    (when (null bmenu)
...
(defun my-buffers-menu-format (buffer)
  (let ((annotation (symbol-value-in-buffer 'buffermenu-annotation buffer)))
    (format "%s%s%*s %s"
